...
public class IntegrationTests {
     ...
     */
    private int countOfIntersection(Intersectable shape, Camera camera, int nX, int nY) {
        int counter = 0;
        for (int i = 0; i < nX; i++) {
            for (int j = 0; j < nX; j++) {
    ...
    void testSphere() {
        ...
        assertEquals(18, countOfIntersection(new Sphere(2,new Point(0, 0, -2.5)),
    ...
    void testPlane() {
        ...
        assertEquals(9,
                countOfIntersection(new Plane(new Point(0, 0, -3), new Point(0, 1, -3), new Point(1, 0, -3)), camera, 3, 3),
                IncorrectNumOfPoints);
        // TC02: The plane is at a small angle and visible by all pixels (9 points)
        assertEquals(9,
                countOfIntersection(new Plane(new Point(0, 0, -3), new Point(0, 1, -2.8), new Point(1, 0, -3)), camera, 3, 3),
                IncorrectNumOfPoints);
