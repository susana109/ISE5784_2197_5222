...
class SphereTest {
    ...
    public void testFindIntersections() {
     /*   Sphere sphere = new Sphere(p100, 1);
        ...
        Sphere sphere = new Sphere(new Point(1,0,0), 2);
// ============ Equivalence Partitions Tests ==============
// TC01: Ray's line is outside the sphere (0 points)
        assertNull(sphere.findIntersections(new Ray(new Point(3, 0, 0),
                        new Vector(1, 0, 0))),
                "Ray's line is not out of sphere");
// TC02: Ray starts before and crosses the sphere (2 points)
        final var result1 = sphere.findIntersections(new Ray(new Point(-1,-1,0),new Vector(1,0,0)));
        assertEquals(2, result1.size(), "Wrong number of points");
        // probleme assertTrue(result1.contains(new Point(-2, -1, 0)), "Intersection point 1 should be (-2, -1, 0");
        //assertTrue(result1.contains(new Point(4, -1, 0)), "Intersection point 2 should be (4, -1, 0)");
// TC03: Ray starts inside the sphere (1 point)
        assertEquals(new Point(1,1,1), sphere.findIntersections(new Ray(new Point(0.5, 0.5, 0),
                new Vector(3, 1, 0).normalize())), "Ray should be from inside sphere");
